<template>
  <div class="space-y-6 bg-gray-50 dark:bg-gray-700">
    <!-- Error -->
    <div
      v-if="itemsError"
      class="bg-red-50 dark:bg-red-900 border border-red-200 dark:border-red-800 rounded-md p-4"
    >
      <div class="flex">
        <div class="flex-shrink-0">
          <svg
            class="h-5 w-5 text-red-400"
            viewBox="0 0 20 20"
            fill="currentColor"
          >
            <path
              fill-rule="evenodd"
              d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z"
              clip-rule="evenodd"
            />
          </svg>
        </div>
        <div class="ml-3">
          <h3 class="text-sm font-medium text-red-800 dark:text-red-200">
            Error loading items data
          </h3>
          <div class="mt-2 text-sm text-red-700 dark:text-red-300">
            <p>{{ itemsError }}</p>
          </div>
        </div>
      </div>
    </div>

    <!-- Loading -->
    <div v-if="itemsLoading" class="flex justify-center items-center py-12">
      <div
        class="animate-spin rounded-full h-12 w-12 border-b-2 border-green-600 dark:border-green-400"
      ></div>
    </div>

    <!-- Loaded -->
    <div v-if="itemsList?.length > 0 && !itemsError && !itemsLoading">
      <ItemsDatabase :items="itemsList" />
    </div>
  </div>
</template>

<script setup>
import { onMounted, ref } from "vue";
import { items } from "../composables/items.js";
import ItemsDatabase from "../components/database/ItemsDatabase.vue";

const itemsList = ref([]);

const { fetchItems, itemsLoading, itemsError } = items();

onMounted(async () => {
  itemsList.value = await fetchItems();
});
</script>
